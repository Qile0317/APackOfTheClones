...
- Add `updateApotc()` generic to update objects generated by `APackOfTheClones` from previous releases. As of this version, all ggplot objects generated from prior versions should be updated with `updateApotc(ggplot_object)`, and several functions have been updated to run this command automatically. The update performed here ensures layer names are all unique in the ggplot.
- Add experimental argument `show_all_links` in `APOTCPlot()` and `vizAPOTC()` to allow plotting of all shared clone links. Attempts to Address GitHub issue [#23](https://github.com/Qile0317/APackOfTheClones/issues/23).
- add `getLastApotcRunId()` as an alias to `getLastApotcDataId()`
- add `getApotcRunIds()` as an alias to `getApotcDataIds()`

## Changes

- To prepare for the upcoming `ggplot2` release where layer naming will be forced to be unique, the naming of legend layers will now ensure each layer name is unique, whereas previously all legend layers had the identical name. This addresses GitHub issue [#25](https://github.com/Qile0317/APackOfTheClones/issues/25).
- add `magrittr` to imports
- remove `sf` from imports
...
- minor documentation & docsite & sourcecode styling cleanup
