...
# get or create apotcdata - assume obj_id is a auto-generated id
getApotcDataIfExistsElseCreate <- function(seurat_obj, obj_id) {
    if (containsApotcRun(seurat_obj, obj_id)) {
        return(getApotcData(seurat_obj, obj_id))
    }

    # assume is a default autogenerated id
    arglist <- deparse_object_id(obj_id)
    actual_metadata_filter <- parse_to_metadata_filter_str(
        arglist$extra_filter, arglist$varargs_list
    )

    getLastApotcData(RunAPOTC(
        seurat_obj,
