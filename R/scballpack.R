...
#' @param rad_decrease_factor numeric. indicates how much the radii of the clones should decrease to add a slight gap between all of them. Defaults to 1 but 0.85-0.95 values are recommended.
...
scballpack <- function(seurat_obj, tcr_df,
  ...
  result_plot <- plot_API(sizes = clone_size_list,
                          ...
                          rad_decrease = rad_decrease_factor,
                          ORDER = ORDER,
                          try_place = try_place,
                          progbar = progbar,
                          repulse = repulse,
