...
#' @title Plot a dataframe of circles
#' @param clusters A dataframe with columns x, y, r, color
#' @param n Number of points to use for each circle
#' @param linetype Type of line to use for the circles
#' @param alpha Transparency of all circles
...
df_full_join <- function(clstr_list, detail = TRUE) {
    ...
    for (i in seq_along(clstr_list)) {
        ...
        df <- df %>% dplyr::full_join(
            # zero indexed
            convert_to_dataframe(clstr_list[[i]], seurat_cluster_index - 1),
            by = cols_to_join_by
        )
    }
