...
To install APackOfTheClones, R version 4.0.0 is required. The free IDE Rstudio is also highly recommended for convenient visualizations and interactions with related objects.

## Dependencies

APackOfTheClones extends the BioConductor scRepertoire v2 package, which itself is an extension of the Seurat package. See their respective installation guides for details. Users of APackOfTheClones should also be familiar with both packages. Briefly, the following should work for most devices:
...
If there are any issues with the CRAN installation (for example, the package had been archived on CRAN for a brief period previously), try
