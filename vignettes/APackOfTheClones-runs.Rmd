...
The following sections are some example usecases
...
APOTCPlot(pbmc, run_id = "first_four", show_labels = TRUE)
```

### Extra Cluster Repulsion

Another possibility is that the default repulsion may have left the clusters too close - here this is corrected - and the labels are moved as well.

```{r, repulse_again, echo = TRUE}
pbmc <- pbmc %>%
    RunAPOTC(run_id = "foo", verbose = FALSE) %>%
    AdjustAPOTC(
        repulse = TRUE, repulsion_threshold = 0.5, verbose = FALSE
    )

APOTCPlot(pbmc, show_labels = TRUE, retain_axis_scales = TRUE)
```
